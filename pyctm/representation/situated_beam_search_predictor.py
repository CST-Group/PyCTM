...
class SituatedBeamSearchPredictor:
    ...
    def beam_search(
        ...
        for i in range(max_len):
            ...
            with ThreadPoolExecutor(max_workers=beam_size) as executor:
                for answer, score, finished, size, states, steps in executor.map(
                    lambda answer: self.process_beam_item(
        ...
        valid_sequences = self.select_possible_plans(
            sdr_idea_deserializer,
            occupiedNodes,
            initial_node,
            closest_nodes_from_goal_tag,
    ...
    def process_beam_item(
        ...
        current_state, current_step, states, steps, allowed_tokens = (
            self.get_allowed_tokens(states, steps)
        )
        ...
        for i in range(top_limit):
            ...
            self.validate_and_extend_steps(
                new_states, new_steps, current_state, next_answer
            )
        ...
        for ys, score, size, _, _, _ in finished_beams:
            try:
                ...
                if self.is_valid_sequence(idea_array):
                    if self.is_valid_plan(
                        action=action,
                        initial_node=initial_node,
                        plan_steps=idea_array,
                        occupiedNodes=occupiedNodes,
                        closest_nodes_from_goal_tag=closest_nodes_from_goal_tag,
                    ):
