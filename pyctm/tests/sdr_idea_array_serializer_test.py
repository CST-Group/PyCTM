...
class SDRIdeaSerializerTest(unittest.TestCase):
    ...
    def init_idea(self):
        ...
        idea.add(Idea(1, "Metallica", "Black Album", 0)).add(
            Idea(10, "Unforgiven", 3.14, 1)
        ).add(Idea(3, "Enter Sadman", "Seek and destroy"))
        idea.add(Idea(4, "Foo Fighters", "The sky's the neighborhood", 0)).add(
            Idea(5, "Pretender", 256)
        )
        idea.add(Idea(6, "Black Sabbath", [3.41, 2.22, 0.23], 1)).add(
            Idea(7, "Paranoid", [34, 18, 10])
        )
        idea.add(Idea(8, "Gun's in Roses", "Sweet child o' mine", 2)).add(
            Idea(9, "November Rain", [-18.0, 1.2, 2, 5.2, -1, 0, 1000])
        )
    ...
    def test_sdr_serialization(self):
        ...
        sdr_idea_serializer = VectorIdeaSerializer(
            total_of_ideas=10, total_of_values=7, default_value=0, dictionary=dictionary
        )
